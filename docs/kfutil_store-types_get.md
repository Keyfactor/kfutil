...
```
kfutil store-types get [-i <store-type-id> | -n <store-type-name>] [-b <git-ref>] [-g | --output-to-integration-manifest] [flags]
```
...
```
  ...
  -n, --name string                      Name of the certificate store type to get.
      --output-to-integration-manifest   Update the integration manifest with the store type. It overrides the store type in the manifest if it already exists. If the integration manifest does not exist in the current directory, it will be created.
```
