...
func (r *ROTAction) toCSV(rowType string) []string {
	...
	case "audit":
		...
		row[headerMap["Alias"]] = r.StoreAlias
		...
		row[headerMap["Machine"]] = r.Machine
		...
		row[headerMap["Deployed"]] = fmt.Sprintf("%t", r.Deployed)
...
type ROTCertCSVEntry struct {
	...
	LastQueried string   `json:"LastQueriedDate"`
}

func (r *ROTCertCSVEntry) getHeaderMap() map[string]int {
	headerMap := make(map[string]int)
	for i, h := range CertHeader {
		headerMap[h] = i
	}
	return headerMap
}

func (r *ROTCertCSVEntry) toCSV() string {
	quotedSubjectName := fmt.Sprintf("\"%s\"", r.SubjectName)
	quotedIssuer := fmt.Sprintf("\"%s\"", r.Issuer)
	return fmt.Sprintf(
		"%d,%s,%s,%s,%s,%v,%s",
		r.ID,
		r.Thumbprint,
