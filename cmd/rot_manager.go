...
func (t *TrustStore) generateMaps() error {
	...
	if t.ThumbPrints == nil {
		log.Debug().Msg("Initializing thumbprint map")
		t.ThumbPrints = make(map[string]bool)
	}
	if t.Serials == nil {
		log.Debug().Msg("Initializing serial number map")
		t.Serials = make(map[string]bool)
	}
	if t.Aliases == nil {
		log.Debug().Msg("Initializing alias map")
		t.Aliases = make(map[string]bool)
	}
	if t.CertIDs == nil {
		log.Debug().Msg("Initializing certificate ID map")
		t.CertIDs = make(map[int]bool)
	}
...
func (t *TrustStore) inventoryItemToCSV(alias string, certID int) string {
	for _, inv := range t.Inventory {
		if inv.Name == alias {
			for _, c := range inv.Certificates {
				if c.Id == certID {
					csvEntry := ROTCertCSVEntry{
						ID:          c.Id,
						Thumbprint:  c.Thumbprint,
						SubjectName: c.IssuedDN,
						Issuer:      c.IssuerDN,
						Alias:       alias,
						//Locations: c.Locations, //todo
						LastQueried: getCurrentTime(""),
					}
					return csvEntry.toCSV()
				}
			}
		}
	}
...
func (t *TrustStore) getInventoryCertificate(alias string, certID int) *api.InventoriedCertificate {
	for _, inv := range t.Inventory {
		if inv.Name == alias {
			for _, c := range inv.Certificates {
				if c.Id == certID {
...
func (t *TrustStore) hasCert(identifier string) bool {
...
func (t *TrustStore) hasCertID(certID int) bool {
	_, ok := t.CertIDs[certID]
	return ok
}

func (t *TrustStore) hasAlias(alias string) bool {
	_, ok := t.Aliases[alias]
	return ok
}

func (t *TrustStore) hasThumbprint(thumbprint string) bool {
	_, ok := t.ThumbPrints[thumbprint]
	return ok
}
...
func (r *RootOfTrustManager) generateAuditReport() error {
	...
	log.Trace().Str("output_file", r.OutputFilePath).
		Strs("remove_data", removeData).
		Msg("Appending remove data to data slice")
...
func (r *RootOfTrustManager) processRemoveCerts(
	...
	for alias, cId := range r.removeCerts {
		log.Debug().Str("alias", alias).Msg("Iterating over stores to determine if cert is deployed")
		for _, store := range r.Stores {
			log.Debug().Str("alias", alias).Str(
				"store_id",
				store.StoreID,
			...
			if store.hasCert(cId) {
				// Cert is deployed to store and will need to be removed
				log.Info().Str("alias", alias).Str("store_id", store.StoreID).Msg("Cert is deployed to store")

				row := store.inventoryItemToCSV(alias, certIDInt)
				log.Trace().Str("alias", alias).Str("row", row).Msg("Appending data row")
				if row != "" {
					data = append(data, row)
				}

				certData := store.getInventoryCertificate(alias, certIDInt)
				if certData == nil {
					log.Error().
						Str("store_id", store.StoreID).
						Str("alias", alias).
						Int("cert_id", certIDInt).Msg("Cert not found in inventory, this shouldn't happen.")
					certData = &api.InventoriedCertificate{}
				}
				...
				action := ROTAction{
					...
					StoreAlias: alias,
					CertDN:     certData.IssuedDN,
					//CertSANs: nil,
					Issuer:     certData.IssuerDN,
					Machine:    store.StoreMachine,
					AddCert:    false,
					RemoveCert: true,
					Deployed:   true,
				}
				_ = csvWriter.Write(action.toCSV("audit"))
				log.Debug().Str("alias", alias).Msg("Adding 'remove' action to actions map")
				actions[alias] = append(actions[alias], action)
			} else {
				log.Info().Str("alias", alias).Str("store_id", store.StoreID).Msg("Cert is not deployed to store")
				...
				_ = csvWriter.Write(action.toCSV("audit"))
				log.Debug().Str("alias", alias).Msg("No action required for this store")
				actions[alias] = append(actions[alias], action)
			}
		}
	}
		...
		for _, a := range action {
			if a.AddCert {
				if !r.IsDryRun {
					...
					certAlias := thumbprint
					if a.StoreAlias != "" {
						certAlias = a.StoreAlias
					}
				...
				if !r.IsDryRun {
					...
					cStore := api.CertificateStore{
						CertificateStoreId: a.StoreID,
						Alias:              a.StoreAlias, //todo: support non-thumbprint aliases
					}
...
func (r *RootOfTrustManager) processStoresFile() error {
	...
	for i, row := range storeEntries {
		...
		} //Todo this should be handled by a receiver method on TrustStore.ToCsv
		...
		merr := r.Stores[row[0]].generateMaps()
		if merr != nil {
			errs = append(errs, merr)
			log.Error().Err(merr).Str("store_id", row[0]).Msg("Error generating maps for store")
		}
	}
...
func (r *RootOfTrustManager) processAddCertsFile() (map[string]string, error) {
	...
	certsToAdd, rErr := r.readCertsFile(r.AddCertsFilePath)
	if rErr != nil {
...
func (r *RootOfTrustManager) processRemoveCertsFile() (map[string]string, error) {
	var errs []error
	log.Debug().Msg(fmt.Sprintf(DebugFuncEnter, "processRemoveCertsFile"))
	...
	certsToRemove, rErr := r.readCertsFile(r.RemoveCertsFilePath)
	if rErr != nil {
...
func (r *RootOfTrustManager) processCSVReportFile() error {
	...
	orchsURL := fmt.Sprintf(
		...
		r.Client.AuthClient.GetServerConfig().Host,
...
func sliceContainsCaseInsensitive(slice []string, str string) bool {
...
func (r *RootOfTrustManager) lookupThumbPrintAliases(tp string) ([]string, error) {
	log.Debug().Msg(fmt.Sprintf(DebugFuncEnter, "lookupThumbPrintAlias"))
	log.Debug().Str("thumbprint", tp).Msg("Looking up alias for thumbprint")
	var aliases []string
	for _, store := range r.Stores {
		if _, ok := store.ThumbPrints[tp]; ok {
			log.Debug().Msg(fmt.Sprintf(DebugFuncExit, "lookupThumbPrintAlias"))
			for _, inv := range store.Inventory {
				//check if tp is in inv.Thumbprints array
				if sliceContainsCaseInsensitive(inv.Thumbprints, tp) {
					log.Debug().Str("thumbprint", tp).Msg("Alias found for thumbprint")
					aliases = append(aliases, inv.Name)
				}
			}
		}
	}
	return aliases, fmt.Errorf("thumbprint '%s' not found in any store", tp)
}

func (r *RootOfTrustManager) readCertsFile(certsFilePath string) (map[string]string, error) {
	...
	for i, entry := range certEntries {
		...
		if alias == "" {
			...
			al, aErr := r.lookupThumbPrintAliases(tp)
			if aErr != nil || len(al) == 0 {
				log.Error().Err(aErr).Str("thumbprint", tp).Msg("Error looking up thumbprint aliases")
				alias = tp
			}
			for _, a := range al {
