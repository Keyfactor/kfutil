...
func Test_StoreTypesGetCmd(t *testing.T) {
	...
	testCmd.SetArgs([]string{"store-types", "get", "--name", "FTP"})
	output := captureOutput(func() {
		err := testCmd.Execute()
		assert.NoError(t, err)
	})
	...
	assert.Equal(t, storeType["ShortName"], "FTP", "Expected short name to be FTP")
}

func Test_StoreTypesGetGenericCmd(t *testing.T) {
	testCmd := RootCmd
	// Attempt to get the AWS store type because it comes with the product
	testCmd.SetArgs([]string{"store-types", "get", "--name", "FTP", "--generic"})
	output := captureOutput(func() {
		err := testCmd.Execute()
		assert.NoError(t, err)
	})
