...
`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if profile == "" && configFile == "" {
			...
			log.Debug().Msg("call: initClient()")
			aConfig := createAuthConfigFromParams(
				authConfig.Servers[profile].Hostname,
				authConfig.Servers[profile].Username,
				authConfig.Servers[profile].Password,
				authConfig.Servers[profile].Domain,
				authConfig.Servers[profile].APIPath,
			)
			log.Debug().Msg("initClient() returned")

			log.Debug().Msg("call: initClient()")
			_, authErr := initClient(configFile, profile, "", "", true, aConfig, false)
			log.Debug().Msg("initClient() returned")

			if authErr != nil {
				log.Error().Err(authErr)
				outputError(authErr, true, outputFormat)
				return authErr
			}
